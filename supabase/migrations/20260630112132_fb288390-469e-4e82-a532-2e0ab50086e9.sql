...
BEGIN
  ...
  -- Do not queue a second brand notification for the same message/status.
  IF EXISTS (
    SELECT 1
  ...
  -- Reserve this notification immediately so retries/double-clicks cannot queue duplicates.
  ...
  VALUES (
    ...
    'pending',
    jsonb_build_object(
      'source', 'message_status_trigger',
      'message_id', NEW.id,
      'brand_id', v_brand_id,
      'recipient_user_id', v_brand_owner_id,
      'status', v_new_status
    )
  );
...
DROP TRIGGER IF EXISTS trg_notify_brand_on_paid_collab_status_change ON public.messages;
CREATE TRIGGER trg_notify_brand_on_paid_collab_status_change
AFTER UPDATE OF brief_data ON public.messages
FOR EACH ROW
WHEN (OLD.brief_data IS DISTINCT FROM NEW.brief_data)
EXECUTE FUNCTION public.notify_brand_on_paid_collab_status_change();