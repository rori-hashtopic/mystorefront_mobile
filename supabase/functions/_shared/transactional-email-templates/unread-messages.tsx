...
const UnreadMessagesEmail = ({ senderName, messagePreview, unreadCount, recipientRole }: Props) => {
  const who = senderName || "Someone";
  const count = unreadCount && unreadCount > 1 ? unreadCount : 1;
  const isBrand = recipientRole === "brand";
  return (
    <Html lang="en" dir="ltr">
      <Head />
      <Preview>
        {who} is waiting on your reply on {SITE_NAME}
      </Preview>
      <Body style={main}>
        <Container style={container}>
          ...
          <Text style={label}>WAITING ON YOU</Text>
          <Heading style={h1}>
            {count > 1 ? `${count} unread messages from ${who}` : `${who} is waiting on your reply`}
          </Heading>
          {messagePreview && (
          ...
          <Text style={text}>
            This conversation has been sitting unread for over an hour. A quick reply keeps things moving.
          </Text>
...
export const template = {
  component: UnreadMessagesEmail,
  subject: (data: Record<string, any>) =>
    data.unreadCount && data.unreadCount > 1
      ? `${data.unreadCount} unread messages from ${data.senderName || "someone"} on ${SITE_NAME}`
      : `${data.senderName || "Someone"} is waiting on your reply on ${SITE_NAME}`,
  displayName: "Unread messages reminder",
  previewData: {
    senderName: "HashTopic",
    messagePreview: "Hey! Just checking in about the campaign — are you still on track for Friday?",
    unreadCount: 2,
...
const previewText = { fontSize: "14px", color: "#1a1d24", fontStyle: "italic" as const, lineHeight: "1.6", margin: "0" };
const button = {
