...
async function sendViaLovable(payload: Record<string, unknown>, apiKey: string): Promise<void> {
  ...
  // Set by send-transactional-email for templates where a human should be
  // able to hit Reply (e.g. a brand inviting a creator).
  if (payload.reply_to) emailPayload.reply_to = payload.reply_to as string;

  if (payload.run_id) emailPayload.run_id = payload.run_id;
  if (payload.idempotency_key) {
    emailPayload.idempotency_key = payload.idempotency_key;
  } else if (!payload.run_id && payload.message_id) {
    emailPayload.idempotency_key = payload.message_id;
  }
  if (payload.unsubscribe_token) emailPayload.unsubscribe_token = payload.unsubscribe_token;

  await sendLovableEmail(emailPayload as any, { apiKey });
}
...
// Cache the vault-stored service-role token across warm invocations.
let cachedVaultServiceKey: string | null = null;
let cachedVaultServiceKeyFetched = false;

async function loadVaultServiceKey(supabase: any): Promise<string | null> {
  ...
  try {
    ...
    // schema, so querying vault.decrypted_secrets directly returns PGRST106
    // ("Invalid schema: vault") and the queue never drains.
    const { data, error } = await supabase.rpc("get_email_queue_service_key");
    if (error) {
...
async function verifyServiceRoleToken(token: string, envServiceRoleKey: string, supabase: any): Promise<boolean> {
  if (envServiceRoleKey && timingSafeEqual(token, envServiceRoleKey)) return true;
  const vaultKey = await loadVaultServiceKey(supabase);
...
Deno.serve(async (req) => {
  ...
  // Byte-compare the incoming token to the known service-role secrets rather
  // than trusting unverified JWT claims (an unsigned {"role":"service_role"}
  // token would otherwise pass). We accept:
  //   • env SUPABASE_SERVICE_ROLE_KEY — used by internal edge-function callers.
  //   • vault `email_queue_service_role_key` — used by the DB triggers and cron
  //     dispatchers (`email_queue_wake` / `email_queue_dispatch`).
  const token = authHeader.slice("Bearer ".length).trim();
  const supabase = createClient(supabaseUrl, supabaseServiceKey) as any;

  const isAuthorized = await verifyServiceRoleToken(token, supabaseServiceKey, supabase);
  if (!isAuthorized) {
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      try {
        ...
        if (isForbidden(error)) {
          ...
          return new Response(JSON.stringify({ processed: totalProcessed, stopped: "emails_disabled" }), {
            headers: { "Content-Type": "application/json" },
          });
        }
